...
class xp(commands.Cog):
    ...
    async def leaderboard(self, ctx): #only one ch use '==' , more use 'in'
        ...
        warns = sorted(warns, key=lambda x: x["count"] ,  reverse=True)

        pages = []
        cmd_per_page = 10
        a = 1
        page = 0
        for i in range(0, len(warns), cmd_per_page):
            message = ""
            next_commands = warns[i: i + cmd_per_page]

            for item in next_commands:
                try:
                    temp = ctx.guild.get_member(item["user_id"])
                    tempxp = item["count"]  
                    message += f"**{a}: {temp.name}** \nTotal XP: {tempxp}\n"
                    a += 1
                except:
                    pass
        ...
        await Pag(title=f"{ctx.guild.name} Rankings", color=0x77dd77, entries=pages, length=1).start(ctx)
        print(len(pages))
